...
app = Client("my_bot", api_id=config.API_ID, api_hash=config.API_HASH, bot_token=config.API_KEY)
...
def buy_premium_command(client, message):
    client.send_message(
        message.chat.id,
        f"Please contact the owner for premium membership: [Owner](tg://user?id={config.OWNER_ID})",
        parse_mode='markdown'
    )
...
@app.on_message(filters.command(["add_user", "remove_user", "stats"]) & (filters.user(config.OWNER_ID) | filters.user(set(map(int, config.ADMIN_IDS.split(','))))))
